...
/// Minimum and maximum padding for each edge.
///
/// The type intentionally preserves the legacy behavior: it does not require or normalize
/// `max >= min`. Enforcing that invariant now could turn an existing layout oddity into a crash or
/// silently rewrite a caller's intent.
public struct MinMaxEdgeInsets: Sendable, Equatable {
